...
export const data = new SlashCommandBuilder()
  .setName('half-access')
  .setDescription('Verify for Half Access role')
...
export async function execute(interaction) {
  ...
  const hasShirt = await ownsShirt(username, config.shirts.half);

  if (hasShirt) {
    const role = interaction.guild.roles.cache.get(config.roles.half);
