...
export const data = new SlashCommandBuilder()
  .setName('exclusive-access')
  .setDescription('Verify for Exclusive Access role')
...
export async function execute(interaction) {
  ...
  const hasShirt = await ownsShirt(username, config.shirts.exclusive);

  if (hasShirt) {
    const role = interaction.guild.roles.cache.get(config.roles.exclusive);
